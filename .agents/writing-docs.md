...
Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/<bucket>/<old>.md` → `docs/<bucket>/<new>.md`) and updates every relative link to it; a skill that moves between `engineering/` and `productivity/` moves its docs file to the matching folder. Skills in `misc/`, `in-progress/`, and `deprecated/` get no page — none of those buckets is promoted. A skill moving *out* of one of them into `engineering/` or `productivity/` gains a page; one moving the other way loses it.

The pages live in this repo and are read on GitHub. **Every link to another skill's page is relative** (`[grill](../productivity/grill.md)`); a link to a file elsewhere in the repo is relative too.
...
**A page carries no install commands.** The one install story is the README's, copied from [the install block](./install-block.md); a page that repeats it drifts.
...
- **Trigger boundary.** The index entry: "reach for this when …". Where the skill is confusable with a sibling, add the other half — "for <X> instead, use [<sibling>](<relative path to its page>)."

## Prerequisites

Optional — include only when the skill needs something in place to be functional; omit the heading entirely otherwise. Covers: a **workspace it writes into** (a stateful skill like `grill` writes `CONTEXT.md` and ADRs; `teach` builds a whole directory — say what it writes and where), **prior setup** (`triage`/`to-spec`/`to-tickets` need `setup-imecoulter-skills` to have configured an issue tracker), or **repo-specific tooling**. A stateless skill that runs anywhere has no prerequisites — drop the section.
...
- **My own sessions.** The questions I actually asked while using the skill, in `~/.claude/projects/*/` transcripts and memory files. A request I typed by hand three times is a question the page owes an answer to.
- **This repo's issues.** `gh issue list --repo imecoulter/skills --search "<skill-name>" --state all`, and upstream's (`--repo mattpocock/skills`) for skills that came from there. A question filed twice is a question the page owes an answer to.
...
- **Role.** Name it: a **chain step** (`grill → to-spec → to-tickets → build → code-review`), a **run-once setup** (`setup-imecoulter-skills`), **periodic maintenance** (`improve-codebase-architecture`, "every few days"), or a **reach-for-it-anytime standalone** (`diagnosing-bugs`, `prototype`, `handoff`). A standalone's map is one honest sentence — far better than omitting the section.
- **Neighbours.** The one or two siblings that matter, each with a because-clause, linked relatively.
- **The map.** Point to [ask](../engineering/ask.md), the router over the whole set, so this page stays a node and never has to redraw the graph.
...
- **Never name the author.** The page is a technical document, not a record of who said what. State the substance as a plain claim about the skill and drop the attribution. Quoting a *user* stays fine — "one user reported …" is evidence about the skill in the wild, and stays anonymous.
...
- `## Where it fits` names the role and links to `ask`.
