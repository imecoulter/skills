...
The **prototype** is the runnable evidence the answer came from, and it is not deleted. It doesn't belong in the app — there is nothing there to maintain and it rots fast — so it is committed to the repo's prototypes folder (e.g. `prototypes/<name>`), never imported, built or deployed, and lands on main with the session's other work. A [context pointer](https://www.aihero.dev/ai-coding-dictionary/context-pointer) to the file goes on the implementation issue. The app stays clean; the exploration stays findable and re-runnable by whoever picks the work up next, and nothing ever points at a branch that might be deleted.
