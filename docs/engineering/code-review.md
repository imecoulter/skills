...
| A whole spec needs building, review included | [build](build.md), which calls this skill itself |
...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-imecoulter-skills](setup-imecoulter-skills.md) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
No. It diffs `<fixed-point>...HEAD`, three-dot, which is measured from the merge-base and excludes staged and working-tree changes. If `build` has not made an interim commit, the work about to be committed is invisible to the review. Commit first, then review, then amend or add a fixup.
...
`code-review` is the review step at the tail of the build chain — `grill → to-spec → to-tickets → build → code-review` — and also stands alone on any branch or PR you point it at.

- [build](build.md) is the closest neighbour: it drives the build and calls this skill as its own closing review before committing.
...
[ask](ask.md) routes across the whole set when you are unsure which skill the situation wants.
