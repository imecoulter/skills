...
| A whole spec needs building, review included | [build](https://aihero.dev/skills-implement), which calls this skill itself |
...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-imecoulter-skills](https://aihero.dev/skills-setup-imecoulter-skills) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
- [build](https://aihero.dev/skills-implement) is the closest neighbour: it drives the build and calls this skill as its own closing review before committing.
