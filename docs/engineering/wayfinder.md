...
| --- | --- |
| A well-scoped feature you can settle in one sitting | [grill](../productivity/grill.md) |
...
| A cleared wayfinder map | [to-spec](to-spec.md), then [to-tickets](to-tickets.md) and [build](build.md) |
...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-imecoulter-skills](setup-imecoulter-skills.md) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
| --- | --- | --- | --- |
| `grilling` | HITL | The default. The question can be settled by talking it through. | [grill](../productivity/grill.md), in a fresh session |
...
| `research` | AFK | A fact outside the working directory is blocking a decision. | [research](research.md), one run per ticket, listed at charting time; the findings land as a file on main |
...
- A session resolves one ticket, posts the answer as a resolution comment, closes it, and leaves one line on the map's *Decisions so far*. It lands whatever it wrote on main through [close](../productivity/close.md), then stops and names the next frontier ticket and the command to take it.
...
Underneath, it is mostly other skills wearing wayfinder's scheduling: [grill](../productivity/grill.md) resolves the default ticket type, [prototype](prototype.md) resolves the tickets that talking cannot, and [research](research.md) runs as a subagent so its reading never lands in your session. [handoff](../productivity/handoff.md) is the bridge in and out — into a map from a conversation that outgrew itself, out of one when a side quest appears mid-session. For anything else, [ask](ask.md) routes over the whole set.
