...
| The whole change fits in one context window | [build](build.md) — skip the tickets |
| Nothing is decided yet | [grill](../productivity/grill.md), then [to-spec](to-spec.md) |
...
`to-tickets` publishes into a tracker, so [setup-imecoulter-skills](setup-imecoulter-skills.md) must have configured one for this repo, along with the triage-label vocabulary. Either kind works: a real tracker like GitHub or Linear, or local markdown files under `.scratch/`, which is supported out of the box.
...
Upstream is [to-spec](to-spec.md), which hands it a settled spec to slice against — keep both in one unbroken context window. Downstream is [build](build.md), which builds one ticket per fresh session, driving [tdd](tdd.md) for the tests and closing with [code-review](code-review.md). When you're unsure which skill or flow fits, [ask](ask.md) routes you.
