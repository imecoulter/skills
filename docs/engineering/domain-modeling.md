...
Type `/domain-modeling`, or the agent reaches for it automatically when a task fits. In practice, automatic invocation is the weakest part of the skill: when `grill` or `wayfinder` say to load it, [models](https://www.aihero.dev/ai-coding-dictionary/model) frequently run the interview and skip this one. If a [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling) session runs and `CONTEXT.md` is untouched at the end, that is what happened — invoke it by name alongside the other skill.
...
| You want the whole plan interrogated before you build | [grill](https://aihero.dev/skills-grill), which drives this skill underneath |
