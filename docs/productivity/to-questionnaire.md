...
| --- | --- |
| Your own head, unsharpened, or the codebase | [grill](grill.md) |
...
**Isn't this just `/grill` in batch mode?**
No, and the distinction is worth holding. `grill` already asks in **rounds** — the whole frontier at once, then recomputed from your answers — so the "give me all the questions at once" need is met there. `to-questionnaire` is about a different axis: not how the questions are delivered, but whose head the answers are in. Answering them yourself faster is `grill`; getting them out of someone else is this.
...
Its neighbour is [grill](grill.md), and the two split on where the answers live: grilling mines you, a questionnaire mines someone else. What comes back is raw material — feed it into another grilling round, or into [grill](grill.md) or [to-spec](../engineering/to-spec.md) if the work is heading for a build. When you're unsure which skill fits the moment, [ask](../engineering/ask.md) routes you.
